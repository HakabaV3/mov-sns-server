...
RSpec.describe "Join", :type => :request do
  ...
  describe 'DELETE /groups/:group_name/join/:user_name' do
    it '通常' do
      expect {
        delete "/api/v1/groups/#{@invitation.group.name}/join/#{@invitation.target.name}",
        {},
        @target_header
      }.to change{ Invitation.count }.by(-1)
    ...
    it '認証はできているが、該当グループに参加していない' do
      expect {
        delete "/api/v1/groups/#{@invitation.group.name}/join/#{@current_user.name}",
        {},
        @token_header
      }.to change{ Invitation.count }.by(0)
    ...
    it '該当グループが存在しない' do
      expect {
        delete "/api/v1/groups/error_group/join/#{@invitation.target.name}",
        {},
        @target_header
      }.to change{ Invitation.count }.by(0)
    ...
    it '認証されたユーザーと、該当ユーザーが一致しない' do
      expect {
        delete "/api/v1/groups/#{@invitation.group.name}/join/error_name",
        {},
        @target_header
      }.to change{ Invitation.count }.by(0)
